...
sync_scripts() {
    ...
    declare -A scripts=(
        ...
        ["helper.sh"]="$BASE/helper.sh"["install.sh"]="$BASE/install.sh"
        ["run.sh"]="$BASE/run.sh"
...
# Check if the dependencies flag exists; if not, install tools and sync scripts
if [[ ! -f "$DEP_FLAG" ]]; then
    setup_tools
    sync_scripts
    touch "$DEP_FLAG" # Mark installation as complete
    echo -e "${G}Installation complete.${NC}"
else
    echo -e "${G}Local installation found. Skipping auto-update.${NC}"
fi
