...
#!/bin/bash
echo "--- [sing-box VLESS TCP+TLS Startup Script - Gaming Optimized] ---"
...
# --- 3. Generate self-signed TLS certificate (fake SNI: playstation.net) ---
echo "Generating self-signed TLS certificate for SNI: $SNI ..."
openssl req -x509 \
    -newkey rsa:2048 \
    -keyout "$KEY_FILE" \
    -out "$CERT_FILE" \
    -days 3650 \
    -nodes \
    -subj "/C=US/ST=California/L=Los Angeles/O=Sony Interactive/OU=PSN/CN=${SNI}" \
    -addext "subjectAltName=DNS:${SNI},DNS:www.${SNI}" \
    2>/dev/null

if [ ! -f "$CERT_FILE" ] || [ ! -f "$KEY_FILE" ]; then
    echo "❌ Certificate generation failed. Make sure openssl is installed."
    exit 1
fi
echo "✅ TLS certificate generated at $CERT_DIR"

# --- 4. Detect public IP (with multiple fallbacks) ---
...
# --- 7. Print VLESS link ---
# allowInsecure=1 is needed on the client side since cert is self-signed
VLESS_LINK="vless://${UUID}@${server_ip}:${SERVER_PORT}?encryption=none&security=tls&type=tcp&sni=${SNI}&allowInsecure=1&fp=chrome#Nour-TLS"
echo ""
echo "=========================================================="
echo "sing-box VLESS Link (TCP + TLS / Gaming Optimized):"
echo "$VLESS_LINK"
echo "=========================================================="
echo ""
echo "⚠️  Client Note: Enable 'Allow Insecure' / 'Skip TLS Verify'"
echo "   since this uses a self-signed certificate."
echo ""
