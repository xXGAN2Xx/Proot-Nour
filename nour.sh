...
# fallback for non-root users on minimal RHEL-based systems.
...
install_dependencies() {
    ...
    # --- RHEL-based System Logic (yum) ---
        ...
        if command -v yumdownloader >/dev/null; then
            ...
            # FALLBACK METHOD: Download a pre-built rootfs for non-root users
            echo -e "${Y}Warning: 'yumdownloader' not found and sudo is not available.${NC}"
            echo -e "${BY}Switching to fallback method: downloading a minimal pre-built environment.${NC}"
            
            local rootfs_url=""
            # These URLs point to CentOS Stream 9 minimal cloud images. They could change.
            case "$ARCH" in
              x86_64) rootfs_url="https://cloud.centos.org/centos/9-stream/x86_64/images/CentOS-Stream-GenericCloud-9-latest.x86_64.tar.gz";;
              aarch64) rootfs_url="https://cloud.centos.org/centos/9-stream/aarch64/images/CentOS-Stream-GenericCloud-9-latest.aarch64.tar.gz";;
              *) error_exit "No fallback rootfs is available for your architecture ($ARCH). Cannot proceed.";;
            esac
            
            echo -e "${Y}Downloading from: ${rootfs_url}${NC}"
            local temp_tar="${HOME}/rootfs.tar.gz"
            curl -L -o "$temp_tar" "$rootfs_url" || error_exit "Failed to download the fallback rootfs."
            
            echo -e "${GR}Extracting rootfs into ${HOME}/.local/ ... (this may take a moment)${NC}"
            # --strip-components=1 removes the top-level directory from the tarball
            tar -xzf "$temp_tar" -C "${HOME}/.local/" --strip-components=1 || error_exit "Failed to extract the fallback rootfs."
            rm "$temp_tar"
            echo -e "${BGR}Fallback environment extracted successfully.${NC}"
        fi
    ...
    # Verify that essential commands are now available in the local PATH
    if ! command -v xz >/dev/null || ! command -v bash >/dev/null; then
        echo -e "${Y}Warning: Essential commands (bash, xz) not found in PATH after installation.${NC}" >&2
        echo -e "${Y}Current PATH: ${PATH}${NC}" >&2
    else
        echo -e "${BGR}Local bash is available at: $(command -v bash)${NC}"
    fi
...
update_scripts() {
    # ... (This function remains unchanged as it is distro-agnostic)
            ...
            if curl -sSLf --connect-timeout 15 --retry 3 -o "$temp_file" "$url"; then
                if [[ ! -f "$local_file" ]] || ! cmp -s "$local_file" "$temp_file"; then
                    if mv "$temp_file" "$local_file" && chmod +x "$local_file"; then
                        ...
                        echo -e "${BR}Update failed for ${dest_path_suffix} (mv/chmod error).${NC}" >&2; rm -f "$temp_file"
                    fi
                else
                    rm "$temp_file"; echo -e "${GR}${dest_path_suffix} is up to date.${NC}"
                fi
            else
                rm -f "$temp_file"; echo -e "${BR}Download failed for ${dest_path_suffix}. Using local version if available.${NC}" >&2
            fi
        ) &
        pids+=($!)
    done
...
if [ -f /etc/os-release ]; then
    . /etc/os-release
    if [[ "$ID" = "alpine" ]]; then
        PKG_MANAGER="apk"; echo -e "${BGR}Alpine Linux detected. Using apk.${NC}"
    elif [[ "$ID" = "amzn" || "$ID_LIKE" == *"rhel"* || "$ID_LIKE" == *"fedora"* || "$ID_LIKE" == *"centos"* ]]; then
        PKG_MANAGER="yum"; echo -e "${BGR}RHEL-based system (Amazon Linux/CentOS/Fedora) detected.${NC}"
    elif [[ "$ID" = "debian" || "$ID_LIKE" == *"debian"* || -f /etc/debian_version ]]; then
        PKG_MANAGER="apt"; echo -e "${BGR}Debian-based system detected. Using apt.${NC}"
    fi
fi
...
if [[ -f "$ENTRYPOINT_SCRIPT" ]]; then
    ...
    # Ensure the locally installed bash is used to run the next script
    if [[ -f "${HOME}/.local/bin/bash" ]]; then
        exec "${HOME}/.local/bin/bash" "./${ENTRYPOINT_SCRIPT##*/}"
    else
